...
function sortAndFilter(maxPriceQuery, minPriceQuery, specialtyQuery, sortQuery) {
    let myurl = "http://localhost:8081/api/cards/filter?"
    if (sortQuery) {
        myurl += `&sort=${sortQuery}`
        document.getElementById("sort").value = sortQuery
    }
    if (specialtyQuery) {
        myurl += `&specialty=${encodeURIComponent(specialtyQuery)}`;
...
document.getElementById("apply-filters").addEventListener("click", () => {
    ...
    window.location.href = `https://localhost:8080/product-listing.html?${params.toString()}`;
});
